...
def runProcess(sCmd):
  ...
  if sCmd == "basics":
    ...
    reportr.loadCSVByDates(lsCustIds, dates, lsTypes=["account", "campaign"], force=True)

    sEmailMsg = """
    The LarsReports basic update script has fully completed.
    ...
    send_mail('LarsReports Basic Data Gathering Completed', sEmailMsg, 'reports@larsmarketing.com',
        ['larsmarketing@arborheights.net','ryan@larsmarketing.com', 'sarah@larsmarketing.com'], fail_silently=False)

  elif sCmd == "details":
    reportr.loadCSVByDates(lsCustIds, dates, force=True, lsTypes=["keyword", "geo"])

    sEmailMsg = """
    The LarsReports details update script has fully completed. 

    """ 

    send_mail('LarsReports Detailed Daily Processing Completed', sEmailMsg, 'reports@larsmarketing.com',
        ['larsmarketing@arborheights.net','ryan@larsmarketing.com', 'sarah@larsmarketing.com'], fail_silently=False)
    ...
    sEmailMsg = """
    The LarsReports pivot update script has fully completed.

    """ 

    send_mail('LarsReports Pivot Load Completed', sEmailMsg, 'reports@larsmarketing.com',
        ['larsmarketing@arborheights.net','ryan@larsmarketing.com', 'sarah@larsmarketing.com'], fail_silently=False)



def main(argv):
  parser = argparse.ArgumentParser(description='Lars Marketing Reporting runner')
  parser.add_argument("cmd", help='Name of cmd to run', choices=["basics", "details", "pivot"])
